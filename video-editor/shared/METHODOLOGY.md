...
2. **Set music.** With no song supplied, generate an original, license-free bed: `node bed.mjs --film <seconds> --drop <seconds>` in the editor folder. Put the drop (the groove entering) on the film's first product reveal; the printed `start` lines it up, and `bed.json` beside the song carries the tempo and start for `beats.mjs`, which snaps the cuts to the music. In the Panels dogfood this was the best-received part of the film. Otherwise import a song you can use, select an excerpt long enough for `film duration × music speed`, and adjust volume and fades. Treat estimated BPM as a starting point: syncopation, intros, and half or double time can fool the analyzer.
3. **Place effects.** Decide what each cue is with [SOUND-DESIGN](SOUND-DESIGN.md): the product's own sounds first, then one sound per role, direction as meaning, gestures under results. Design missing sounds with `sfx.mjs`. Put a sound at the visual event's frame. Audition click, arrival, typing, and confirmation cues against the picture. Keep small gestures quieter than decisive actions. Use the editor's sound replacement and undo controls when changing a family of cues.
...
Measure because you cannot hear. The bundled Cuelume sounds are UI-quiet (peaks around -12 to -21 dBFS): at editor music volume 0.85 every cue sat 15-40 dB under the bed, and even at 0.3 the small press and release clicks were only 0-4 dB over it. The default music volume is 0.22 with cues near full; the report flags any cue under +4 dB. Raise small gestures to 0.8-1.0 rather than leaving them at the "quiet" levels the editor suggests for listening. Then run `node listen.mjs --video final.mp4 --recipe final.mix.json --edit edit.json --drop <s>`: it checks loudness and true peak, audio/picture alignment, dead air, a faded ending, and whether each cue (or group of cues within 120 ms) produces a transient that stands out from the music around it at its frame, with a timing allowance of 50 ms plus the sound's own attack. It writes a spectrogram over the waveform marking cuts, cues (red when masked), and the drop: look at it. On the Panels film it flagged a click that does not stand out from the music around it, and (in a remix) the reveal cue swallowed by the drop it lands on. A mix that measures well can still sound wrong, so a human listen through the final frame stays the last step.
