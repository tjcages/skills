...
1. Run the product and record the actual interaction when it relies on browser layout, CSS, portals, real-time state, or animation that the renderer cannot reproduce. Screen capture is valid film material; match resolution and frame rate, and use a repeatable fixture when possible. `capture/record.mjs` does this at full resolution with the product's real timing (see [staging](staging.md), Recording the live product). A fixture must reproduce the host environment the UI ships into, such as a CSS reset, root font size, or theme provider, or the capture shows a state no user sees.
...
## Product bugs found while filming

Filming exercises a product harder than its demo page does, so bugs surface. When one does, film a truthful path around it (a different but real sequence that shows the feature working), note the deviation in the provenance record, and file the bug with the product. Never fake the intended behaviour, and never hide the workaround from the person who asked for the film.
