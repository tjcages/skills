...
Create the ledger before the first assignment. Use the existing tracker when one exists. For a short run, keep it in the active plan. For a long or compaction-prone run, persist it in a scoped project document such as `docs/orchestration/<slug>.md` when repository writes are authorized.
...
The ledger is the resumable source of truth. Chat summaries and worker confidence are not substitutes.
